...
const clearTmuxSession = () => {
  curTmuxSession = undefined;
  curTmuxWindows = [];
};

// Current tmux session
const setTmuxSession = () => {
  const tmuxCmd = "/usr/local/bin/tmux list-panes -F '#{pane_tty} #{session_name}' | awk '{print $2}'";
  // exec(`/usr/local/bin/tmux display-message -p '#S'`, {}, (err, stdout, stderr) => {
  exec(tmuxCmd, {}, (err, stdout, stderr) => {
    if (err) {
      console.log(`exec err: ${err}`);
      return;
    }
    curTmuxSession = stdout;
    console.log(`curTmuxSession: ${curTmuxSession}`);
  });
};

const setTmuxWindows = () => {
  const tmuxCmd = `${TMUX} list-windows -F '{"name":"#W", "active":#{window_active}}'`;
  exec(tmuxCmd, {}, (err, stdout, stderr) => {
    if (err) {
      console.log(`exec err: ${err}`);
      return;
    ...
    curTmuxWindows = stdout.split('\n').filter(str => str.length > 3).map(window => JSON.parse(window));
    console.log(`curTmuxWindows: ${JSON.stringify(curTmuxWindows)}`);
  });
...
exports.decorateHyper = (Hyper, { React }) => {
  return class extends React.Component {
    constructor(props) {
      super(props);
      this.state = {
        windows: curTmuxWindows,
        session: curTmuxSession,
        folder: curCwd,
        branch: curBranch,
        remote: curRemote,
        dirty: repoDirty,
    ...
    render() {
      ...
      const hasSession = this.state.session ? ' item_active' : '';
      // const hasWindows = this.state.windows ? ''
      ...
      //     {[1, 2].map(num => (
      //       <span className='item_session center'>{this.state.session} {num}</span>
      //     ))}
      //   </div>
      // );
        ...
        React.createElement(Hyper, Object.assign({}, this.props, {
          customChildren: React.createElement('footer', { className: 'footer_footer' },
            React.createElement('div', {className: `container`}, 
              React.createElement('span', { className: `item_session center`}, hasSession ? this.state.session : ''),
              this.state.windows.map(window => React.createElement('span', {className: `item_window center ${window.active ? 'window_active' : ''}`}, hasSession ? window.name : ''))
